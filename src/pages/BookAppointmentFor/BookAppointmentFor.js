...
class BookAppointmentForPage extends Component {
  state = {
    isLoading: true,
    doctor: new Object(),
    allAppointments: [],
  ...
  componentDidMount() {
    axios
      ...
      .then(doctorResponse => {
        this.setState({ isLoading: false, doctor: doctorResponse.data, name: doctorResponse.data.name });
        document.getElementById('doctorName').innerHTML = doctorResponse.data.name;
        ...
        .then(result => {
          ...
          if (this.state.hasCurrentAppointment === true) {
            ...
            const appointmentData = {
              id: this.state.doctor._id,
              name: this.state.doctor.name,
              phoneNumber: this.state.doctor.phoneNumber,
              specialization: this.state.doctor.specialization,
              totalExperience: this.state.doctor.totalExperience,
              workingDays: this.state.doctor.workingDays,
              patientName: this.state.patientName,
              patientPhone: this.state.patientPhone,
              patientAge: this.state.patientAge,
              patientGender: this.state.patientGender,
            ...
            request
              .then(result => {
                this.setState({ isLoading: false });
                //this.props.history.replace('/appointment/book');
  ...
  bookAppointmentPatient = event => {
    ...
    //isOverlapAppt = this.validateOverlapAppointmentDoctor(this.state.doctor._id, this.state.appointmentDate, this.state.appointmentTime, appointmentData);
    this.validateOverlapAppointmentDoctor(this.state.doctor._id, this.state.appointmentDate, this.state.appointmentTime, appointmentData);
  };

  resetValues = event => {
    this.setState({ doctor: new Object() });
                    ...
                    <tbody>
                      ...
                      {/* <tr>
                        <td colSpan="2" style={{ textAlign: "left" }}>
                          Please click on Book Appointment after clicking on Save Details to confirm the appointment.
                        </td>
                      </tr>
                      <tr>
                        <td style={{ textAlign: "right" }}>
                          <input type="submit" id="btnSaveDetails" className="blueBtn" value="Save Details"
                            onClick={this.savePatientDetails} />
                        </td>
                        <td style={{ textAlign: "left" }}>
                          <input type="submit" id="btnBookAppointment" className="blueBtn" value="Book Appointment"
                            onClick={this.bookAppointmentPatient} />
                        </td>
                      </tr> */}
                      <tr>
                        <td style={{ textAlign: "right" }}>
...
export default BookAppointmentForPage;